...
export default function SearchableSelect({
    ...
    const filtered = options.filter(
        o => String(o[labelKey] ?? '').toLowerCase().includes(search.toLowerCase())
    );

    const selected = options.find(o => o[valueKey] == value) ?? null;
    ...
    const handlePick  = opt   => { onChange(opt[valueKey]); setOpen(false) }
                ...
                >
                    ...
                    <div className="max-h-56 overflow-y-auto py-1">
                        {filtered.length === 0 ? (
                            ...
                            filtered.map(opt => {
                                const isActive = value == opt[valueKey];
                                return (
                                    <button
                                        key={opt[valueKey]}
                                        type="button"
                                        ...
                                        style={{
                                            background: isActive ? 'rgba(57,96,169,0.06)' : 'transparent',
                                        }}
                                    >
                                        <div
