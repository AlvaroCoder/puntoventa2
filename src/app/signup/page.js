...
import {
  getVerificarDocumento,
  getVerificarEmail,
} from "../../../API/user/getConnections";
...
export default function SignUpPage() {
      ...
      switch (currentStep) {
        ...
        case 3:
          if (!formData.businessName) {
            setErrors((prev) => ({ ...prev, businessName: "Nombre del negocio requerido" }));
            hasError = true;
          }
          if (!formData.ruc) {
            setErrors((prev) => ({ ...prev, ruc: "RUC requerido" }));
            hasError = true;
          }
          if (!formDataEnterprise.rubro_id) {
            setErrors((prev) => ({ ...prev, rubro: "Por favor selecciona un rubro" }));
            hasError = true;
          }
          break;
        ...
        case 4:
          if (!formData.storeCount) {
            setErrors((prev) => ({ ...prev, storeCount: "Selecciona una escala" }));
            hasError = true;
          }
          break;
      }
  ...
  const handleClickCategory = (id) => {
    setFormDataEnterprise((prev) => ({ ...prev, rubro_id: id }));
  };
        ...
        return (
          <div className="h-full flex flex-col md:flex-row gap-6 animate-in slide-in-from-right-4 duration-300">
            <div className="w-full space-y-4">
              ...
              <label className="text-sm font-bold text-[#333333] ml-1 block mb-2">
                Selecciona tu Rubro
              </label>
              {errors?.rubro && (
                <p className="text-red-500 text-xs">{errors.rubro}</p>
              )}
              <GridSelectCardRubro
      ...
      case 4:
        return (
          <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
            ...
            {errors?.storeCount && (
              <p className="text-red-500 text-xs text-center">{errors.storeCount}</p>
            )}
            ...
            <div className="bg-[#198E7B]/10 p-4 rounded-xl flex gap-3 items-start">
              ...
              <div>
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Crearemos{" "}
                  {formData.storeCount === "1" ? "tu tienda principal" : "tu panel multi-sede"}{" "}
        ...
        <div className="w-full md:w-80 bg-[#1F4363] p-8 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-12">
              <div className="w-10 h-10 bg-[#FF821E] rounded-xl flex items-center justify-center text-white font-extrabold shadow-lg">
                360
              </div>
                  ...
                  <div key={step.id} className="flex gap-4 group">
                    <div className="flex flex-col items-center">
                      ...
                      {index !== STEPS.length - 1 && (
                        <div
                          className={`w-0.5 h-full mt-2 rounded-full transition-colors duration-500 ${isCompleted ? "bg-[#198E7B]" : "bg-white/10"}`}
                        ></div>
                      )}
                    </div>
        ...
        <div className="flex-1 flex flex-col relative">
          <div className="md:hidden p-4 border-b bg-gray-50 flex justify-between items-center">
            ...
            <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                className="h-full bg-[#FF821E] transition-all duration-300"
