    ...
    return (
      <div className="space-y-5 animate-in slide-in-from-right-4 duration-300">
        <div className="my-4">
          ...
          <p className="text-gray-500 text-sm">Comencemos con tu usuario</p>
        </div>
              ...
              {results.map(({ key, label, passed }) => (
                <div key={key} className="flex items-center gap-2">
                  {passed ? (
                    <CheckCircle2
                      size={13}
                      className="text-[#1B8D7C] shrink-0"
                    />
                  ) : (
        ...
        {showConfirm && (
          <div className="space-y-2 animate-in slide-in-from-bottom-2 duration-300">
            ...
            {confirmPassword.length > 0 && (
              <div
                className={`flex items-center gap-1.5 text-xs font-medium px-1 transition-all duration-200 ${
